...
public class Historial {
    ...
    public void guardarHistorial(Crear_Rutina crearRutina, Instructor instructor) {
        try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream("historial.historial"))) {
            oos.writeObject(crearRutina);
            oos.writeObject(instructor);
            JOptionPane.showMessageDialog(null, "Historial y rutina guardados correctamente en historial.historial.");
        } catch (IOException e) {
    ...
    public void cargarHistorial() {
        try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream("historial.historial"))) {
